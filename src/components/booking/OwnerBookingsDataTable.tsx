...
import { ChevronDownIcon, ChevronFirstIcon, ChevronLastIcon, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, MoreHorizontal, Eye, CheckCircle, LogIn, LogOut, Loader2 } from "lucide-react"
import { cn } from "@/lib/utils"
import { bookingService, BookingStatus, PaymentStatus, Booking } from "@/services/bookingServices"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function OwnerBookingsDataTable({ bookings, onAction, page, totalPages, onPageChange, totalItems, pageSize }: OwnerBookingsDataTableProps) {
  ...
  const columns: ColumnDef<Booking>[] = useMemo(() => [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            <div className="font-medium">{b.vehicle?.brand} {b.vehicle?.model}</div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span>{u?.first_name} {u?.last_name}</span>
      },
      size: 200,
    },
    {
      ...
      cell: ({ row }) => {
        const b = row.original
        return (
          <span className="whitespace-nowrap">{formatDate(b.start_date)} - {formatDate(b.end_date)}</span>
        )
      },
      size: 220,
    },
                    ...
                    <DropdownMenuItem
                      ...
                      onSelect={(e) => {
                        ...
                        const descriptions: Record<string, string> = {
                          approve: "Approve this booking. The customer will be notified and payment processing will proceed.",
                          checkout: "Confirm that the renter has picked up the vehicle and the rental period has started.",
